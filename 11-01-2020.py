...
############## Binarize labels , new compared to 09-01-2020 #####################
    
def make_bin_ds(df, lb):
    
    labels = df.label
    bin_labels = lb.transform(labels)
    
    ds = tf.data.Dataset.from_tensor_slices((df.path,bin_labels))
    
    return ds
    
train_ds = make_bin_ds(df_train, lb).map(process_path, num_parallel_calls=AUTOTUNE)
test_ds = make_bin_ds(df_test, lb).map(process_path, num_parallel_calls=AUTOTUNE)
val_ds = make_bin_ds(df_val, lb).map(process_path, num_parallel_calls=AUTOTUNE)
...
csv_logger = CSVLogger(os.path.join(logdir,'11-01-2020.log'),append=True)
